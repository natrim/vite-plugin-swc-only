...
export function registerExportsForReactRefresh(filename, moduleExports) {
  for (const key in moduleExports) {
    ...
    if (isLikelyComponentType(exportValue)) {
      // export is required to avoid key collision https://github.com/vitejs/vite-plugin-react/issues/116
...
export function validateRefreshBoundaryAndEnqueueUpdate(
  ...
  if (hasExports && allExportsAreComponentsOrUnchanged) {
    ...
    return "Could not Fast Refresh. Your file should only export React components.";
  }
}
